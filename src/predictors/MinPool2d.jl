...
julia> predictor = MathOptAI.MinPool2d((2, 2); input_size = (2, 4, 1))
MinPool2d((2, 4, 1), (2, 2), (2, 2), (0, 0))
...
│ └ moai_MinPool2d[2]
└ constraints [2]
  ├ moai_MinPool2d[1] - min(min(min(x[1,1], x[2,1]), x[1,2]), x[2,2]) = 0
  └ moai_MinPool2d[2] - min(min(min(x[1,3], x[2,3]), x[1,4]), x[2,4]) = 0
...
2-element Vector{NonlinearExpr}:
 min(min(min(x[1,1], x[2,1]), x[1,2]), x[2,2])
 min(min(min(x[1,3], x[2,3]), x[1,4]), x[2,4])

julia> formulation
ReducedSpace(MinPool2d((2, 4, 1), (2, 2), (2, 2), (0, 0)))
├ variables [0]
└ constraints [0]
```
"""
struct MinPool2d <: AbstractPredictor
...
function (f::MinPool2d)(model::JuMP.AbstractModel, x::Vector)
    ...
    return JuMP.@expression(
        ...
        minimum(X[sH*(h-1)+m, sW*(w-1)+n, c] for m in 1:kH, n in 1:kW),
    )
end

function output_size(f::MinPool2d, input_size::NTuple{3,Int})
    (Hin, Win, C) = f.input_size
    (kH, kW), (pH, pW), (sH, sW) = f.kernel_size, f.padding, f.stride
    Hout = floor(Int, (Hin + 2 * pH - kH) / sH + 1)
    Wout = floor(Int, (Win + 2 * pW - kW) / sW + 1)
...
function add_predictor(
    ...
    Y = predictor(model, x)
    y = add_variables(model, x, length(Y), "moai_MinPool2d")
...
function add_predictor(
    model::JuMP.AbstractModel,
    predictor::ReducedSpace{MinPool2d},
    x::Vector,
)
