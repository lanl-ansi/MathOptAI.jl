...
An [`AbstractPredictor`](@ref) that represents a mean pooling layer.
...
```
"""
struct AvgPool2d <: AbstractPredictor
...
function (f::AvgPool2d)(model::JuMP.AbstractModel, x::Vector)
    ...
    return JuMP.@expression(
        ...
        sum(X[sH*(h-1)+m, sW*(w-1)+n, c] for m in 1:kH, n in 1:kW) / (kH * kW),
    )
end

function output_size(f::AvgPool2d, input_size::NTuple{2,Int})
    return output_size(f, (input_size..., 1))
end

function output_size(f::AvgPool2d, input_size::NTuple{3,Int})
    (Hin, Win, C) = f.input_size
    (kH, kW), (pH, pW), (sH, sW) = f.kernel_size, f.padding, f.stride
    Hout = floor(Int, (Hin + 2 * pH - kH) / sH + 1)
    Wout = floor(Int, (Win + 2 * pW - kW) / sW + 1)
...
function add_predictor(
    ...
    Y = predictor(model, x)
    y = add_variables(model, x, length(Y), "moai_AvgPool2d")
...
function add_predictor(
    model::JuMP.AbstractModel,
    predictor::ReducedSpace{AvgPool2d},
    x::Vector,
)
